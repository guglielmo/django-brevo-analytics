...
2. Get your Supabase JWT Secret from: https://supabase.com/dashboard/project/fvuhpocdeckmbdgiebfy/settings/api
   - Scroll to "JWT Settings" section
   - Click "Reveal" on "Legacy JWT Secret" (or use JWT Signing Keys if migrated)
3. Get your anon key from the same page (Project API keys → anon public)
4. Run the script and follow the prompts
5. Save the generated JWT token
...
BREVO_SUPABASE_ANON_KEY=<your-anon-key>
BREVO_JWT=<your-generated-jwt-token>
```

**Where to find these:**
- `BREVO_SUPABASE_ANON_KEY`: Dashboard → Settings → API → Project API keys → anon public
- `BREVO_JWT`: Generated by `04_generate_jwt.py` script
